...
- **`actualCheckOut`** (`LocalDateTime`, nullable) foi adicionado em F09 — registra quando o check-out de fato ocorreu (D-22). Junto com `actualCheckIn`, viabiliza as listagens F10 (hóspede no hotel = `actualCheckIn` preenchido e `actualCheckOut` nulo) e F11 (sem check-in = `actualCheckIn` nulo), ainda não implementadas.
- F15 (tela de criação), F10/F11 (listagens) — não implementados.
