...
- **Ajuste (D-39, 2026-07-26)**: check-out no mesmo dia calendário do check-in agora cobra o mínimo de 1 diária (a do dia do check-in) em vez de lançar `IllegalArgumentException` — descoberto por F33 (E2E de UI real de [[Check-in e Check-out]]). A validação continua rejeitando apenas quando `checkOut` não é cronologicamente depois de `checkIn`.
