...
- **D-35** — [[Categoria de Quarto]] (F35): máscara de moeda nos 7 campos de preço usa `mask="separator.2"` + `outputTransformFn`/`inputTransformFn` (inputs do `NgxMaskDirective`) para manter o `FormControl` tipado como `number` — sem isso, o valor mascarado vira string e quebraria o contrato de `RoomCategoryPricesRequest` (`Map<DayOfWeek, BigDecimal>` no backend, que espera um número JSON, não uma string com vírgula). Confirmado via smoke test manual (Playwright ad-hoc) que o payload PUT enviado continua numérico. Gotcha de UX descoberto no smoke test: `separator.2` não desloca decimais estilo calculadora (digitar "12000" vira "R$ 12.000", doze mil) — o atendente precisa digitar a vírgula decimal explicitamente ("120,00") para obter cêntimos, mesmo comportamento de digitar o valor por extenso à mão.
- **D-36** — [[Quarto]] (F36): o compilador do Angular (`ngc`/`ng build`) não consegue inferir corretamente o tipo de `room` (de `*matCellDef="let room"`) dentro do conteúdo projetado de `<mat-select-trigger>` quando indexado num `Record<RoomStatus, string>` -- gera erro `NG3`/TS7053 mesmo com `$any()`. Contornado movendo o lookup para métodos tipados no componente (`statusLabel(status: RoomStatus)`, `statusClass(status: RoomStatus)`) chamados do template em vez de indexar o record diretamente ali. Efeito colateral zero no comportamento -- é só uma peculiaridade do type-checking de template dentro de conteúdo projetado por componentes de terceiros.
